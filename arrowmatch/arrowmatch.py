...
if os.path.exists(f"{args.restore_dir}/final_checkpoint"):
    ...
    if args.obfus == "translinkguard":  
        ...
        #st()
        # 测试无恢复的结果
        if args.no_recover_test == "true":
            restore_model = obfus_model
        else:
        ...
        set_seed()
        # make sure the weight_dir are changed to TSQP_results
        model = AutoModelForSequenceClassification.from_pretrained(args.weight_dir, num_labels=num_labels)
        set_seed()
        pre_model =  AutoModelForSequenceClassification.from_pretrained(args.model, num_labels=num_labels)
        # 混淆模型并测试混淆后的模型性能
...
#删掉args.output_dir和args.obfus_dir
os.system(f"rm -rf {args.output_dir}")
os.system(f"rm -rf {args.obfus_dir}")