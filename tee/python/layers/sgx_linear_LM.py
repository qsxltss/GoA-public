...
class SGXLinearLM(SecretLayerBase):
    batch_size = None
    InputShape = None
    WeightShape = None
    OutputShape = None
    ...
    def __init__(
        self, sid, LayerName, EnclaveMode, batch_size, max_seq_len, n_output_features, 
    ...
    def init_shape(self):
        ...
        if self.n_input_features is None:
            ...
            self.InputShape = self.DerInputShape = [self.batch_size, self.max_seq_len, self.n_input_features]
        self.OutputShape = self.DerOutputShape = [self.batch_size, self.max_seq_len, self.n_output_features]
    ...
    def init(self, start_enclave=True):
        ...
        if self.EnclaveMode is ExecutionModeOptions.Enclave:
            ...
            """
            Use batch_size * max_seq_len to simulate the batch_size of original linear layer
            """
            self.sgx_linear_init(
                ...
                self.n_input_features, self.n_output_features
            )
        else:
    ...
    def forward(self):
        with NamedTimerInstance(f"S{self.sid}: {self.LayerName} Forward", verbose_level=VerboseLevel.LAYER):
            if self.EnclaveMode is ExecutionModeOptions.Enclave:
                # raise NotImplementedError("SGX Linear LM in enclave mode is not checked")
                print(f"{self.LayerName} Forward")
                ...
                self.set_gpu(
                    "output", 
                    self.ForwardFunc(input.type(SecretConfig.dtypeForCpuOp))
                )
