...
class SecretRMSNormLayer(SecretLayerBase):
    def __init__(
        self, sid, LayerName, EnclaveMode, 
        dim, eps=1e-6, link_prev=True, link_next=True,
        ...
        self.ForwardFuncName = "RMSNorm"
        self.BackwardFuncName = "RMSNorm"
        self.dim = dim
        self.eps = eps
        
        self.ForwardFunc = RMSNorm
        self.PlainFunc = RMSNorm
    ...
    def forward(self):
        with NamedTimerInstance(
            ...
            if self.EnclaveMode == ExecutionModeOptions.Enclave:
                raise RuntimeError(
                    "RMSnorm in SGX is not checked, recommend to use GPU for RMSnorm"
                )
